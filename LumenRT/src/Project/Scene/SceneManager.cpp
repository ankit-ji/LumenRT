...
namespace LumenRT
{
	...
	LRT_GUID SceneManager::CreateScene(const std::string& name) {
	...
	void SceneManager::DeleteScene(LRT_GUID guid) {
		if (m_InRuntimeSimulation) {
		...
		if (m_OpenSceneGuid == guid) {
			m_OpenSceneGuid = LRT_GUID::INVALID;
	...
	bool SceneManager::SetOpenSceneGuid(LRT_GUID guid) {
		auto scene = find(guid);
		if (!scene && guid != LRT_GUID::INVALID) { // allow setting to LRT_GUID::INVALID (no scene selected)
	...
	LRT_GUID SceneManager::GetOpenSceneGuid() const {
	...
	void SceneManager::SaveScenesToFolder(const std::filesystem::path& folderpath) const {
		...
		for (const auto& scenepath : FindFilesInFolder(folderpath, SCENE_FILE_EXTENSION)) {
			LRT_GUID guid = ExtractGuidFromScenepath(scenepath);
