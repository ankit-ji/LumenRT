...
namespace LumenRT 
{
	...
	void AssetsPanel::OnImGuiRender() {
		...
		if (ImGui::BeginPopup("Add Menu", ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            if (ImGui::MenuItem(ICON_FA_CUBE " Asset...")) {
				...
				if (!assetPath.empty()) {
					LRT_GUID guid = assetManager->ImportAsset(assetPath.string());
				}
            }
			ImGui::EndPopup();
		}
        ...
        if (ImGui::BeginTable("##AssetTable", 2, ImGuiTableFlags_Resizable | ImGuiTableFlags_NoSavedSettings)) {
            ...
            ImGui::BeginChild("LeftPanel", ImVec2(0, ImGui::GetContentRegionAvail().y), false);
            {
                ...
                m_SelectedTileGuid = LRT_GUID::INVALID;
                }
    ...
    void AssetsPanel::DrawSceneTile(LRT_GUID guid, const char* title) {
		DrawGenericTile(guid, title, ICON_FA_CIRCLE_NODES, DNDPayloadTypes::SCENE);
	}

    void AssetsPanel::DrawAssetTile(LRT_GUID guid, const char* title) {
		if (!m_ProjectManager->ProjectIsOpen()) {
			return;
		}
    ...
    void AssetsPanel::DrawGenericTile(LRT_GUID guid, const char* title, const char* icon, const char* dndPayloadType) {
    ...
    void AssetsPanel::DrawTileInfo() {
        ...
        // No asset selected
        if (m_SelectedTileGuid == LRT_GUID::INVALID) {
            return;
        }
        ...
        if (std::shared_ptr<Scene> scene = sceneManager->find(m_SelectedTileGuid)) {
			...
			if (ImGui::Button(scnBtnLabel.c_str(), scnBtnSize)) {
                if (currentSceneOpen) {
                    sceneManager->SetOpenSceneGuid(LRT_GUID::INVALID);
                }
                else {
            ...
            if (ImGui::Button(ICON_FA_TRASH "##ClearSkyboxBtn")) {
                scene->skyboxName = "";
                scene->skyboxGuid = LRT_GUID::INVALID;
            }
            ...
            if (ImGui::Checkbox("##bootSceneCheckbox", &isBoot)){
                if (isBoot) {
                    ...
                    m_ProjectManager->SetBootSceneGuid(LRT_GUID::INVALID);
                }
            }
            theme.PopColor();
