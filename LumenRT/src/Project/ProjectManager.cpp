...
namespace LumenRT
{
	...
	std::optional<ProjectFile> LoadProjectFile(const std::filesystem::path& projectFilepath) {
		...
		try {
			...
			projectFile.bootSceneGuid = static_cast<LRT_GUID>(node["bootSceneGuid"].as<uint64_t>());
			YAML::Node rsNode = node["RuntimeRenderSettings"];
	...
	bool ProjectManager::OpenProject(const std::filesystem::path& projectfilePath) {
		...
		{
			LOG_ENGINE_ERROR("OpenProject: invalid .lrtproj file selected: {}", projectfilePath.string());
			return false;
		}
		...
		if (m_ProjectFile.bootSceneGuid != LRT_GUID::INVALID) {
