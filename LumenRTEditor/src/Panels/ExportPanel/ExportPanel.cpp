...
namespace LumenRT
{
	...
	void LumenRT::ExportPanel::OnImGuiRender() {
		...
		bool sceneSet = (m_ProjectManager->GetBootSceneGuid() != LRT_GUID::INVALID);
		...
		DragDropWidget(
			...
			[&](const DNDPayload& payload) {
				...
				m_ExportSuccessful = false;
				m_ExportFailed = false;
				sceneSet = (m_ProjectManager->GetBootSceneGuid() != LRT_GUID::INVALID);
			},
			theme,
