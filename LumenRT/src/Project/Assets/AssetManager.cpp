...
namespace LumenRT
{
    ...
    std::optional<AssetMetaFile> LoadMetaFile(const std::filesystem::path& metafilePath) {
        ...
        try {
            ...
            metafile.guid = (LRT_GUID)root["Guid"].as<uint64_t>();
    ...
    LRT_GUID AssetManager::ImportAsset(const std::filesystem::path& assetpath) {
	...
	void AssetManager::SaveAssetPoolToFolder(const std::filesystem::path& folderpath) const {
		...
		for (const auto& metapath : FindFilesInFolder(folderpath, ASSET_META_FILE_EXTENSION)) {
			...
			LRT_GUID guid = maybeMetafile->guid;
		...
		for (const auto& [guid, metadataPair] : m_AssetPool->Metadata) {
			...
			if (metadataExtension && std::filesystem::exists(metadataExtension->sourcePath)) {
				...
				// save .lrmeta in the project root next to .lrtproj file with filename same as the original asset + .lrmeta extension
	...
	bool AssetManager::LoadAssetFile(const std::filesystem::path& assetpath, LRT_GUID guid) {
	...
	bool AssetManager::LoadMesh(const std::filesystem::path& assetpath, LRT_GUID guid) {
	...
	bool AssetManager::LoadTexture(const std::filesystem::path& assetpath, LRT_GUID guid, int channels) {
		auto timerStart = std::chrono::high_resolution_clock::now();
