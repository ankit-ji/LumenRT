...
namespace LumenRT
{
	...
	EntityHandle Scene::CreateEntityWithGuid(LRT_GUID guid, const std::string& name) {
		entt::entity entityID = m_Registry->create();
		EntityHandle entity(entityID, m_Registry);
	...
	std::shared_ptr<Scene> LoadSceneFile(const std::filesystem::path& scenepath) {
		...
		scene->guid        = static_cast<LRT_GUID>(getScalar(root["SceneGuid"], uint64_t(0), "SceneGuid"));
		scene->name        = getScalar(root["SceneName"], std::string("Untitled Scene"), "SceneName");
		scene->skyboxGuid  = static_cast<LRT_GUID>(getScalar(root["SkyboxGuid"], uint64_t(0), "SkyboxGuid"));
		...
		if (!entitiesNode || !entitiesNode.IsSequence()) {
			...
			for (auto entityNode : entitiesNode) {
				...
				auto guid = static_cast<LRT_GUID>(getScalar(entityNode["IDComponent"], (uint64_t)LRT_GUID{}, "IDComponent")); // give a random guid if missing
				...
				if (entityNode["MeshComponent"]) {
					...
					mc.guid       = static_cast<LRT_GUID>(getScalar(mnode["MeshGuid"], uint64_t(0), "MeshGuid"));
				}
