...
#define EDITOR_THEME_FILE_EXTENSION ".lrttheme"
...
namespace LumenRT
{
	...
	public: 
		...
		void LoadDefaultLight(); // not used (exported in repo as DefaultLight.lrttheme)
...
template <>
struct YAML::convert<LumenRT::EditorTheme> {
	static YAML::Node encode(const LumenRT::EditorTheme& rhs) {
		YAML::Node node;
		for (size_t i = 0; i < LumenRT::EditorCol_COUNT; i++) {
			node[LumenRT::EditorColStrings[i]] = rhs[static_cast<LumenRT::EditorCol_>(i)];
		}
		return node;
	}

	static bool decode(const YAML::Node& node, LumenRT::EditorTheme& rhs) {
		if (!node.IsMap()) return false;
		for (size_t i = 0; i < LumenRT::EditorCol_COUNT; i++) { 
			if (node[LumenRT::EditorColStrings[i]]) { 
				rhs[static_cast<LumenRT::EditorCol_>(i)] = node[LumenRT::EditorColStrings[i]].as<ImVec4>();
			}
		}
		return true;
	}
