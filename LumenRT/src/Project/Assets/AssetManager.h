...
namespace LumenRT
{
	...
	public:
		...
		std::unordered_map<LRT_GUID, MetadataPair> Metadata; // (polymorphic type)
		...
		template <typename T>
		std::shared_ptr<T> find(const LRT_GUID& guid) const {
	...
	struct AssetMetaFile {
		AssetMetaFile(LRT_GUID guid = LRT_GUID::INVALID, std::filesystem::path sourcePath = "")
			: guid(guid), sourcePath(std::move(sourcePath)) {
		}

		LRT_GUID guid = LRT_GUID::INVALID;
	...
	public:
		...
		/// - Loads the asset.
		/// - Returns the new asset LRT_GUID on success
		/// - Returns LRT_GUID::INVALID if unsuccessful
		LRT_GUID ImportAsset(const std::filesystem::path& assetpath);
		bool RemoveAsset(LRT_GUID guid);
	...
	private:
		...
		bool LoadAssetFile(const std::filesystem::path& assetpath, LRT_GUID guid);

		// Loaders
		bool LoadMesh(const std::filesystem::path& assetpath, LRT_GUID guid);
		bool LoadTexture(const std::filesystem::path& assetpath, LRT_GUID guid, const int channels = 4);
	};
} 